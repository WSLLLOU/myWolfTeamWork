#ifndef SENTRY_MODULE_MODEL_TENSORRTX_YOLOV5_HPP_
#define SENTRY_MODULE_MODEL_TENSORRTX_YOLOV5_HPP_
...
class Yolov5TRT {
    private:
        ...
        Logger gLogger;                         // static

        char *trtModelStream;
        size_t size;
        float prob[BATCH_SIZE * OUTPUT_SIZE];   // static
        IRuntime *runtime;
        ICudaEngine *engine;
...
int Yolov5TRT::get_width(int x, float gw, int divisor) {
    return int(ceil((x * gw) / divisor)) * divisor;
}

int Yolov5TRT::get_depth(int x, float gd) {
    if (x == 1) return 1;
    int r = round(x * gd);
...
ICudaEngine* Yolov5TRT::build_engine(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
...
ICudaEngine* Yolov5TRT::build_engine_p6(unsigned int maxBatchSize, IBuilder* builder, IBuilderConfig* config, DataType dt, float& gd, float& gw, std::string& wts_name) {
    INetworkDefinition* network = builder->createNetworkV2(0U);
    // Create input tensor of shape {3, INPUT_H, INPUT_W} with name INPUT_BLOB_NAME
    ITensor* data = network->addInput(INPUT_BLOB_NAME, dt, Dims3{ 3, INPUT_H, INPUT_W });
    assert(data);
...
void Yolov5TRT::APIToModel(unsigned int maxBatchSize, IHostMemory** modelStream, bool& is_p6, float& gd, float& gw, std::string& wts_name) {
    // Create builder
...
// 推理
void Yolov5TRT::doInference(IExecutionContext &context, cudaStream_t &stream, void **buffers, float *output, int batchSize)
{
...
// 构造函数
Yolov5TRT::Yolov5TRT(std::string& model_name) {
    cudaSetDevice(DEVICE);

    // 获取其后缀 "wts" or "engine"
	std::string::size_type  iPos = model_name.find_last_of('.') + 1;
    std::string             suffix_str = model_name.substr(iPos, model_name.length() - iPos);
    ...
    if (suffix_str == "wts") {
        ...
        // 
        engine_name = model_name.substr(0, iPos);
        engine_name += "engine";
        // 判断 engine 是否存在, 若存在则不进行engine的构造
    ...
    } else if (suffix_str == "engine") {
        engine_name = model_name;
    } else {
        std::cerr << "请输入正确的模型文件路径" << std::endl;
    }
...
std::vector<Yolo::Detection> Yolov5TRT::operator()(cv::Mat& img) {
    if (img.empty()) {
...
#endif // SENTRY_MODULE_MODEL_TENSORRTX_YOLOV5_HPP_